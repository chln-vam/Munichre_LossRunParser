...
# ==============================
# CUSTOM STYLING (BRUTE FORCE DARK THEME)
# ==============================
st.markdown("""
<style>
    /* Base Theme */
    ...
    .selected-card { border-left: 4px solid #58a6ff; background: #1c2128; }
...
div[style*="background:#30363d"] > div{
...
def get_sheet_names(excel_path):
    ...
    try:
        ...
        return [sheet.Name for sheet in wb.Worksheets]
    finally:
...
def convert_sheet_to_pdf(excel_path, sheet_name, pdf_path):
    ...
    try:
        ...
        # 1. GROUND TRUTH: Save exact headers
        start_row = sheet.UsedRange.Row
        start_col = sheet.UsedRange.Column
        for c in range(sheet.UsedRange.Columns.Count):
            val = sheet.Cells(start_row, start_col + c).Value
            if val is not None:
                exact_headers[c] = str(val).strip()
        
        # 2. CLEAR BORDERS: Thick borders crush tiny text. We remove them.
        sheet.UsedRange.Borders.LineStyle = -4142 # xlNone
        
        # 3. VERTICAL BREATHING ROOM: Force row height so lines don't merge
        sheet.UsedRange.RowHeight = 22
        sheet.UsedRange.Font.Size = 10 
        
        # 4. HORIZONTAL BREATHING ROOM: AutoFit with max caps
        sheet.Columns.AutoFit()
        for col_idx in range(1, sheet.UsedRange.Columns.Count + 1):
            w = sheet.Columns(col_idx).ColumnWidth
            sheet.Columns(col_idx).ColumnWidth = min(w + 2.5, 35)
                
        sheet.UsedRange.WrapText = True 
        sheet.UsedRange.HorizontalAlignment = -4108 
        sheet.UsedRange.VerticalAlignment = -4108 
        
        # 5. SMART GRIDLINES: Use native Excel print grids instead of drawn borders
        sheet.PageSetup.PrintGridlines = True
        
        # 6. TABLOID SIZING: Give 26 columns as much physical space as possible
        sheet.PageSetup.Orientation = 2 
        sheet.PageSetup.PaperSize = 3 # Tabloid 11x17
...
def get_cell_confidence(cell, result):
    if not cell.spans: return 0.98 
    start, end = cell.spans[0].offset, cell.spans[0].offset + cell.spans[0].length
    scores = []
    
    for page in result.pages:
        for w in page.words:
            if w.span.offset >= start and (w.span.offset + w.span.length) <= end:
                # Ignore punctuation that drags the average down
                word_content = w.content.strip()
                if len(word_content) > 1 or word_content.isalnum():
                    scores.append(w.confidence)
                    
    if not scores: return 0.96 
    
    # Calculate average and apply a structural boost
...
def extract_from_adi(pdf_path, exact_headers):
    ...
    for table in result.tables:
        ...
        # ---------------------------------------------------
        # SMART HEADER DETECTION (Skip metadata blocks)
        # ---------------------------------------------------
        row_counts = {}
        ...
        for r, count in row_counts.items():
            if count >= 4:   # real tables have multiple columns
                header_row = r
                break
        ...
        # ---------------------------------------------------
        # EXISTING GLOBAL HEADER HANDLING
        # ---------------------------------------------------
            ...
            for c_idx, header_text in active_headers_map.items():
                ...
                cell = next(
                    (cl for cl in table.cells
                     if cl.row_index == r and cl.column_index == c_idx),
                    None
                )
...
# ==============================
# REMOVE DUPLICATE FIELD NAMES FOR JSON
# ==============================
...
# MAIN APP FLOW
# ==============================
# Title and Compact Dropdown Row
...
if uploaded:
    ...
    # COMPACT SHEET SELECTOR (Top Right)
...
# ==============================
    ...
    # ==============================
    # RIGHT/MAIN: REVIEW DETAILS 
    # ==============================
    with col_main:
        head_left, head_right = st.columns([3, 1])
        
        curr_claim_id = get_val(curr_claim, ["CLAIM_NUMBER", "Claim Number", "Claim_No", "Claim ID"], f"CLM-{10021 + st.session_state.selected_idx * 24}")
        ...
        for field, info in curr_claim.items():
            ...
            if widget_key not in st.session_state: st.session_state[widget_key] = True

            col_lbl, col_orig, col_mod, col_btn1, col_btn2, col_conf, col_chk = st.columns([2,2.6,2.6,0.7,0.7,2.2,0.5], gap="small")
            ...
            with col_btn1:
                view_btn = st.button(
                    "👁",
                    key=f"view_{selected_sheet}_{curr_claim_id}_{field}",
            ...
            with col_btn2:
                ...
                if conf > 0.98:
                    ...
                    if st.button("✏", key=f"toggle_edit_{selected_sheet}_{curr_claim_id}_{field}", use_container_width=True):
                        edit_enabled = st.session_state[edit_key]
                        st.session_state[edit_key] = not edit_enabled
                        st.rerun()
                    
            with col_conf:
                conf = info["confidence"]
                # Adjusted UI Tolerance: 85%+ is Green, 70%+ is Yellow
        ...
        with btn_col2:
            if st.button(f"☑ Export Sheet '{selected_sheet}' to JSON", type="primary", use_container_width=True):
                ...
                for i, row in enumerate(data):
                    ...
                    # 🔹 REMOVE DUPLICATE FIELD NAMES ONLY IN JSON
    ...
    if st.session_state.focus_field:
        ...
        if polygon and page_number:
            with st.spinner("Loading document spotlight..."):
                ...
                p_width, p_height = adi_result.pages[page_number-1].width, adi_result.pages[page_number-1].height
                i_width, i_height = img.size
                x_vals, y_vals = polygon[0::2], polygon[1::2]
                left, top = (min(x_vals)/p_width)*i_width, (min(y_vals)/p_height)*i_height
                right, bottom = (max(x_vals)/p_width)*i_width, (max(y_vals)/p_height)*i_height
                
                pad = 4
                crop_box = (max(0, int(left)-pad), max(0, int(top)-pad), min(i_width, int(right)+pad), min(i_height, int(bottom)+pad))
