...
# LOGO
# ==============================
logo_path = r"C:\Users\LakshmiNarayanaCheru\OneDrive - ValueMomentum, Inc\Documents\Demos\Munichre_LossRunParser\ValueMomentum_Logo.jpg"
...
def show_lineage_popup(field: str, info: dict, pdf_path: str):
    ...
    st.markdown(f"### 🔗 Lineage Trace for `{field}`")

    lineage_data = {
        "Field Name": field,
        "Extracted Value": info.get("value"),
        "Modified Value": info.get("modified"),
        "Confidence Score": info.get("confidence"),
        "Source Page": info.get("page"),
...
Lineage shows the **complete traceability** of the extracted field.

• Which **document** the value came from  
• Which **page** it was extracted from  
• The **exact bounding region** detected by Azure AI  
• The **model used for extraction**  
• The **confidence score assigned by the AI model**

This ensures **auditability and explainability** for insurance claims processing.
""")
    ...
    if selected_sheet not in st.session_state.sheet_cache:
        with st.spinner(f"Building PDF & analyzing '{selected_sheet}' with Azure Intelligence..."):
            ...
            st.session_state.sheet_cache[selected_sheet] = {
                "data": data, "adi_result": adi_result, "pdf_path": pdf_path,"sheet_title": sheet_title,
