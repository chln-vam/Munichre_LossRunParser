...
# ==============================
def convert_sheet_to_pdf(file_path: str, sheet_name: str, pdf_path: str):
    exact_headers = {}
    ext = os.path.splitext(file_path)[1].lower()
    ...
    if len(all_rows) >= 1:
        sheet_title = " ".join([c for c in all_rows[0] if c]).strip()
    if len(all_rows) >= 2:
        sheet_meta = " ".join([c for c in all_rows[1] if c]).strip()

    data_rows = all_rows[2:]
    if not data_rows:
        raise ValueError("No claim data rows detected.")

    header_row_idx = 0
    for i, row in enumerate(data_rows):
        if len([v for v in row if str(v).strip()]) >= 4:
            header_row_idx = i
            break
    ...
    table_data = []
    for row in data_rows[header_row_idx:]:
    ...
    return exact_headers, sheet_title, sheet_meta
...
def show_eye_popup(field: str, info: dict, pdf_path: str, adi_result):
    ...
    if polygon and page_number and pdf_path:
        ...
        crop = (
            max(0, int(left) - pad),
            max(0, int(top) - pad),
            min(i_w, int(right) + pad),
            min(i_h, int(bottom) + pad),
        )
...
@st.dialog("Field Lineage")
def show_lineage_popup(field: str, info: dict, pdf_path: str):
    ...
    <div style="
        ...
        margin-bottom: 16px;
    ">
        <div style="font-size:13px; color:#8b949e; text-transform:uppercase; margin-bottom:4px;">
            🔗 Lineage Trace
        </div>
        <div style="font-size:20px; font-weight:bold; color:#58a6ff;">
            {field}
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    lineage_items = [
        ...
        ("Extraction Model",      "Azure Document Intelligence – prebuilt-layout"),
        ("Extraction Timestamp",  datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
    ]

    for label, value in lineage_items:
        st.markdown(f"""
        <div style="
            display: flex;
    ...
    <div style="
        ...
        margin-top: 14px;
    ">
        <div style="font-size:12px; color:#58a6ff; font-weight:bold; text-transform:uppercase; margin-bottom:10px;">
            What This Means
        </div>
        <div style="font-size:13px; color:#c9d1d9; line-height:1.8;">
            Lineage shows the <span style="color:#58a6ff; font-weight:bold;">complete traceability</span> of the extracted field.<br><br>
            • Which <span style="color:#e6edf3; font-weight:bold;">document</span> the value came from<br>
            • Which <span style="color:#e6edf3; font-weight:bold;">page</span> it was extracted from<br>
            • The <span style="color:#e6edf3; font-weight:bold;">exact bounding region</span> detected by Azure AI<br>
            • The <span style="color:#e6edf3; font-weight:bold;">model</span> used for extraction<br>
            • The <span style="color:#e6edf3; font-weight:bold;">confidence score</span> assigned by the AI model
        </div>
        <div style="
            margin-top: 12px;
            font-size: 11px;
            ...
            padding-top: 10px;
        ">
            ✅ Ensures auditability and explainability for insurance claims processing.
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with col_title:
    with col_title:
        st.markdown('<div class="main-title" style="padding-top:40px;">🛡️ TPA Loss Run Parser Analyzer</div>', unsafe_allow_html=True)
...
if uploaded:
    ...
    if selected_sheet not in st.session_state.sheet_cache:
        with st.spinner(f"Building PDF & analyzing '{selected_sheet}' with Azure Intelligence..."):
            ...
            try:
                exact_headers, sheet_title, sheet_meta = convert_sheet_to_pdf(
                    excel_path, selected_sheet, pdf_path
                )
    ...
    # ── SHEET HEADER BANNER ───────────────────────────────────────────────
    sheet_title = active.get("sheet_title", "")
    sheet_meta  = active.get("sheet_meta", "")
    ...
    <div style="
        ...
        margin-bottom:20px;
    ">
        <div style="font-size:11px;color:#8b949e;text-transform:uppercase;margin-bottom:6px;">
            📄 LOSS RUN REPORT
        </div>
        <div style="font-size:20px;font-weight:600;color:white;margin-bottom:6px;">
            {sheet_title}
        </div>
    ...
    with col_main:
        ...
        # ── FIELD TABLE HEADER ─────────────────────────────────────────────
        hc = st.columns([2, 2.6, 2.6, 0.6, 0.6, 2.2, 0.5, 0.7])
        ...
        for field, info in curr_claim.items():
            ...
            cl, co, cm, ce, cb, cc, cx, clg = st.columns([2, 2.6, 2.6, 0.6, 0.6, 2.2, 0.5, 0.7], gap="small")

            with cl:
                st.markdown(
                    f"<div style='height:40px;display:flex;align-items:center;"
                    f"color:#c9d1d9;font-size:12px;font-weight:bold;text-transform:uppercase;'>{field}</div>",
                    unsafe_allow_html=True,
                )
            with co:
                st.text_input(
            ...
            with clg:
                if st.button("🔗", key=f"lin_{selected_sheet}_{curr_claim_id}_{field}", use_container_width=True):
                    show_lineage_popup(field, info, pdf_path)
    ...
    if st.session_state.focus_field:
        ...
        if polygon and page_number and pdf_path:
            with st.spinner("Loading spotlight..."):
                ...
                st.markdown(
                    f"<p style='color:white;font-size:16px;'>Spotlight: "
                    f"<span style='color:#58a6ff;font-weight:bold;'>{field}</span> "
                    f"(Page {page_number})</p>",
                    unsafe_allow_html=True,
                )
